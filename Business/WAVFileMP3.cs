 ...
 *
 *2015-04-02 menahem covert to mp3 file (using NAudio DLL)
 */
    ...
    public class WAVFileMP3
    {
        ...
        public String Open(WAVFileMode pMode)
        {
            ...
            try
            {
                ...
                Mp3FileReader reader = new Mp3FileReader(mFilename);
                
                WaveStream waveStream = WaveFormatConversionStream.CreatePcmStream(reader);
                WaveFileWriter waveFileWriter = new WaveFileWriter(mFileStream, waveStream.WaveFormat);
        ...
        public void Create(String pFilename, bool pStereo, int pSampleRate, short pBitsPerSample, bool pOverwrite)
        {
            ...
            //try
            {
                ...
                using (Mp3FileReader reader = new Mp3FileReader(mFilename))
                {
                    using (WaveStream waveStream = WaveFormatConversionStream.CreatePcmStream(reader))
                    {
                        using (WaveFileWriter waveFileWriter = new WaveFileWriter(mFileStream, waveStream.WaveFormat))
                        {
                            byte[] bytes = new byte[waveStream.Length];
                            waveStream.Position = 0;
                            waveStream.Read(bytes, 0, (int)waveStream.Length);
        ...
        /// <returns></returns>
        public bool FormatMatches(WAVFileMP3 pWAVFile)
        {
            bool retval = false;
        ...
        public static WAVFormat GetAudioFormat(String pFilename)
        {
            ...
            WAVFileMP3 audioFile = new WAVFileMP3();
        ...
        public static void AdjustVolume_Copy(String pSrcFilename, String pDestFilename, double pMultiplier)
        {
            ...
            WAVFileMP3 srcFile = new WAVFileMP3();
            ...
            if (retval == "")
            {
                ...
                WAVFileMP3 destFile = new WAVFileMP3();
        ...
        public static void MergeAudioFiles(String[] pFileList, String pOutputFilename, String pTempDir)
        {
            ...
            WAVFileMP3[] scaledAudioFiles = new WAVFileMP3[pFileList.GetLength(0)];
            String filename = "";               // For the scaled-down WAV filename
            WAVFileMP3 inputFile = new WAVFileMP3();
            WAVFileMP3 outputFile = new WAVFileMP3();
            for (int i = 0; i < pFileList.GetLength(0); ++i)
            {
                ...
                // audio file with it.
                scaledAudioFiles[i] = new WAVFileMP3();
            ...
            foreach (WAVFileMP3 audioFile in scaledAudioFiles)
            {
        ...
        static private bool SamplesRemain(WAVFileMP3[] WAVFileArray)
        {
        ...
        private MemoryStream mFileStream; // For reading the audio file
