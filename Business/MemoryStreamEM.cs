 ...
 *
 * 2015 -04-02 menahem chnage to work with MemoryStream instead FileStream
 * 
 * 
 * 
 */
...
namespace EricOulashin
{
    ...
    public class MemoryStreamEM
    {
        ...
        public String Open(WAVFileMode pMode)
        {
            ...
            try
            {
                //mFileStream = File.Open(mFilename, FileMode.Open, FileAccess.Read);
                mFileStream = new MemoryStream();
        ...
        public MemoryStream Create(String pFilename, bool pStereo, int pSampleRate, short pBitsPerSample, bool pOverwrite)
        {
            ...
            //catch (Exception exc)
            {
                //sthrow new WAVFileException(exc.Message, "WAVFile.Create()");
            }
        }
        ...
        /// <param name="pSampleRate">The sample rate of the audio file (in Hz)</param>
        /// <param name="pBitsPerSample">The number of bits per sample (8 or 16)</param>
        public MemoryStream Create(String pFilename, bool pStereo, int pSampleRate, short pBitsPerSample)
        {
            return Create(pFilename, pStereo, pSampleRate, pBitsPerSample, true);
        }
        ...
        /// <returns></returns>
        public bool FormatMatches(MemoryStreamEM pWAVFile)
        {
            bool retval = false;
        ...
        public static WAVFormat GetAudioFormat(String pFilename)
        {
            ...
            MemoryStreamEM audioFile = new MemoryStreamEM();
        ...
        static public bool IsWaveFile(String pFilename)
        {
            ...
            if (File.Exists(pFilename))
            {
                try
                {
                    FileStream fileStream = File.Open(pFilename, FileMode.Open, FileAccess.Read);
        ...
        public static void AdjustVolume_Copy(String pSrcFilename, String pDestFilename, double pMultiplier)
        {
            ...
            MemoryStreamEM srcFile = new MemoryStreamEM();
        ...
        public static void MergeAudioFiles(String[] pFileList, String pOutputFilename, String pTempDir)
        {
            ...
            MemoryStreamEM[] scaledAudioFiles = new MemoryStreamEM[pFileList.GetLength(0)];
            ...
            for (int i = 0; i < pFileList.GetLength(0); ++i)
            {
                ...
                // audio file with it.
                scaledAudioFiles[i] = new MemoryStreamEM();
            ...
            foreach (MemoryStreamEM audioFile in scaledAudioFiles)
            {
            ...
            if (highestBitsPerSample == 8)
            {
                byte highestSampleVal = MemoryStreamEM.HighestSampleValue_8bit(pOutputFilename);
                byte maxValue = byte.MaxValue / 4 * 3;
                multiplier = (double)maxValue / (double)highestSampleVal;
            }
            else if (highestBitsPerSample == 16)
            {
                short finalMixFileHighestSample = MemoryStreamEM.HighestSampleValueAs16Bit(pOutputFilename);
        ...
        static private bool SamplesRemain(MemoryStreamEM[] WAVFileArray)
        {
        ...
        public MemoryStream mFileStream; // For reading the audio file
